...
class ProductsWindow(TableWindow):
    def __init__(self, user):
        ...
        super().__init__("Product", columns, "💊 Учет лекарств", user)
    
    def load_data(self):
        """Загрузка данных с расчетом статуса"""
        conn = get_connection()
        if not conn:
        ...
        cursor = conn.cursor()
        try:
            # Используем названия колонок из вашей таблицы Product
            cursor.execute("""
            ...
            for row_idx, row in enumerate(data):
                ...
                # Цена
                if row[5]:
                    self.table.setItem(row_idx, 5, QTableWidgetItem(f"{float(row[5]):.2f} ₽"))
                else:
                    self.table.setItem(row_idx, 5, QTableWidgetItem("0.00 ₽"))
        ...
        try:
            ...
            if count > 0:
                reply = QMessageBox.question(self, "Подтверждение",
                    f"У этого товара есть {count} списаний. Удалить всё равно?",
