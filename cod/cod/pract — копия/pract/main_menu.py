...
class MainMenu(QWidget):
    ...
    def init_ui(self):
        ...
        btn_change_password.setStyleSheet("background-color: #f39c12; color: white; padding: 15px;")
    ...
    def get_statistics(self):
        ...
        try:
            ...
            cursor.execute("SELECT COUNT(*) FROM Product WHERE Expiration_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '30 days';")
            expiring = cursor.fetchone()[0]
            cursor.execute("SELECT COUNT(*) FROM Write_downs WHERE Date >= CURRENT_DATE - INTERVAL '30 days';")
            writeoffs = cursor.fetchone()[0]
            
            return [
                f"Всего товаров: {total}",
                f"Просрочено: {expired}",
                f"Истекает через 30 дней: {expiring}",
                f"Списаний за 30 дней: {writeoffs}"
            ]
